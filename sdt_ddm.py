...
if __name__ == "__main__":
    # 1) Print the README (you already have this)
    readme_path = Path(__file__).parent / 'README.md'
    if readme_path.exists():
        print(readme_path.read_text())
    else:
        print("** No README.md found—continuing without it. **")
    
    # 2) Load & display your SDT summary
    csv_path = Path(__file__).parent / 'data' / 'responses.csv'  # adjust to your file
    sdt_df = read_data(csv_path, prepare_for='sdt', display=True)

    # 3) Build the hierarchical SDT model
    model = apply_hierarchical_sdt_model(sdt_df)
    
    # 4) Sample from the posterior (this will take a minute!)
    with model:
        trace = pm.sample(draws=500, tune=500, chains=2, target_accept=0.9)
    
    # 5) Print a quick ArviZ summary
    print("\n— PyMC Posterior Summary —")
    print(az.summary(trace, var_names=['mean_d_prime','mean_criterion']))

    # 6) Prepare & draw delta‐plots for participant #1
    delta_df = read_data(csv_path, prepare_for='delta plots', display=False)
    draw_delta_plots(delta_df, pnum=1)
    print(f"\nDelta plot saved to output/delta_plots_1.png")